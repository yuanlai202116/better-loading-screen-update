...
public interface LoadGameSteps {
    ...
    // Merged: replaces phases 3-5 (LoadModel + StitchTexture + BakeModel)
    interface LoadAssets {
        StatusIdentifier<ParentTask> TASK = StatusIdentifier.of("Loading Assets");
        StatusIdentifier<SteppedTask> LOAD = StatusIdentifier.of("Loading");
        StatusIdentifier<SteppedTask> EXTRACT = StatusIdentifier.of("Extracting");
        StatusIdentifier<SteppedTask> STITCH = StatusIdentifier.of("Stitching");
        StatusIdentifier<SteppedTask> BAKE = StatusIdentifier.of("Baking");
    }
    ...
    static ParentTask loadAssets() {
        ParentTask task = loadGame().parent(LoadGameSteps.LoadAssets.TASK);
